...
int main(){
	std::string answer="";
	for (int i = 1013; i < 3333; i += 2){//primes are always odd, cut work in half
		if (prime(i)){
...
bool prime(int answer){// this will hopefully never contemplate whether 1 or 2 is prime
...
bool permutation(int one, int two){
	...
	for (int i = 0; i < str_one.length(); i++){//str_one.length SHOULD == str_two length
		bool flag = false;
