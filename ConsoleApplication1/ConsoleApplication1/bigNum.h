...
public:
	void initialize(unsigned int i){     //easier than putting it in the constructor
		myNum[0] = i;
		int k = 0;
	...
	bigNum(){      //default constructor
		...
		capacity = 1000;
		for (int i = 0; i < capacity; i++){//force ALL non used items to be 0
			myNum[i] = -1;
		}
		digitAmount = 1;
	}
	int getSize()const{ //needed in some problems
		return digitAmount;
	}
	void setSize(int in){// not used
		digitAmount = in;
	}
	int sumDigits(){//used in some problems
		int answer = 0;
	...
	std::string toString()const {   //ah yes Java
	...
	void print(){//incase include iostream is bad
		printf("%c", toString());
	}
	int overFlow(int i){
		int k;
		for (k = i; myNum[k] >= 10 || myNum[k + 1] != -1; k++){// built to handle the overflow
			//if the next most signifigant digit is defined, then add the overflow to it, otherwise, initialize it
			if (myNum[k] >= 10){//if it DOES overflow
	...
	bigNum& operator*=(const bigNum& rhs){
		...
		for (int i = 0; i < rhs.digitAmount; i++){
			if (i == 0){//for multiplication of 10^0 , also for initialization
				for (int j = 0; j < /*this->digitAmount */ digitAmount; j++){
					temp_two.myNum[j] = myNum[j] * rhs.myNum[i];
				}//endfor
			}//endif
			else{//all places above 10^0
				for (int j = 0; j < /*this->digitAmount */ digitAmount; j++){
