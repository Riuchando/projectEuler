...
int main(){
	...
	for (myit = primeNumber.begin(); myit != primeNumber.end(); myit++){
		...
		if (circular(primeNumber, noOverwrite)){ //see how many prime numbers are circular
			// std::cout<<(*myit)<<'\t';
			answer++;
		}
...
bool circular(std::vector<int>& myVector, long multiple){
	if (multiple<10){// don't waste time
		return true;
	}
	...
	for (int i = 0; i<mulString.size(); i++){
		if (multiple>100){
			...
			for (int j = 0; j<mulString.size() - 1; j++){//rotate all the values
				/*
				example 12345:
		...
		for (tempit = myVector.begin(); tempit != myVector.end(); tempit++){
			...
			if (fmod(rotatedNum, (*tempit)) == 0){//is divisible, will leave on next term
				return false;
			}
			if ((*tempit)> upperbound){//short curcuit, only need to check values up to sqrt of curr number
				break;
			}
		}
