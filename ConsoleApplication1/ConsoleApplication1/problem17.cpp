...
int main(){
	...
	for (int i = 1; i <= 1000; i++){
		...
		if (i / 100 >= 1 && i / 100 <= 9){ //done this way for readability
			int hundreds = i / 100;
		...
		if ((i % 100) / 10 == 1){//teens  eg 115 = one hundred and FIFTEEN
			wordTotal += teen[i % 10];
		}
		else if ((i % 100) / 10 > 1){// double digit numbers eg 125= one hundred and TWENTY FIVE
			wordTotal += doubledigit[((i % 100) / 10)-2]; 
			wordTotal += single[i % 10];
}
		else {	//(i%100) /10 ==0 eg 101 = one hundred and ONE
			wordTotal += single[i % 10];
		}
	}
	std::cout << wordTotal << '\n';
